# -*- coding: utf-8 -*-
"""
Created on Mon Aug  5 04:46:52 2024
...
def load_khop(path="./datasets/cora/", dataset="cora", khops =1):
    ...
    features = F.FloatTensor(np.array(features.todense()))
    labels = F.LongTensor(np.where(labels)[1])
    adj = sparse_mx_to_torch_sparse_tensor(adj)

    idx_train = F.LongTensor(idx_train)
    idx_val = F.LongTensor(idx_val)
...
def load_data(path="./Datasets/Cora/", dataset="cora"):
    ...
    idx_train = F.LongTensor(idx_train)
    idx_val = F.LongTensor(idx_val)
    idx_test = F.LongTensor(idx_test)
...
def load_dataT(path="./datasets/cora/", dataset="cora"):
    ...
    edges_unordered = np.genfromtxt("{}{}.cites".format(path, dataset),
                                    dtype=np.int32)
    edges = np.array(list(map(idx_map.get, edges_unordered.flatten())), # flatten
                     dtype=np.int32).reshape(edges_unordered.shape) 
    adj = sp.coo_matrix((np.ones(edges.shape[0]), (edges[:, 0], edges[:, 1])),
                        shape=(labels.shape[0], labels.shape[0]),
                        dtype=np.float32)
...
def sparse_mx_to_torch_sparse_tensor(sparse_mx):
    ...
    indices = F.from_numpy(
        np.vstack((sparse_mx.row, sparse_mx.col)).astype(np.int64))
    values = F.from_numpy(sparse_mx.data)
    shape = F.Size(sparse_mx.shape)
    return F.sparse.FloatTensor(indices, values, shape)
...
def load_dataP(dataset, degree_as_tag):
    ...
    for g in g_list:
        ...
        g.edge_mat = F.LongTensor(edges).transpose(0,1)
    ...
    for g in g_list:
        g.node_features = F.zeros(len(g.node_tags), len(tagset))
