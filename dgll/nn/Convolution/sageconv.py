...
class NeighborAggregator(F.nn.Module):
    ...
    def __init__(self, input_dim, output_dim, use_bias=False, aggr_method="mean"):
        ...
        self.weight = F.Parameter(F.Tensor(self.input_dim, self.output_dim))

        if use_bias:
            self.bias = F.Parameter(F.Tensor(self.output_dim))

        self.reset_parameters()

    def reset_parameters(self):
        F.init.kaiming_uniform_(self.weight)
        if self.use_bias:
            F.init.zeros_(self.bias)
    ...
    def forward(self, neighbor_feature):
        ...
        neighbor_hidden = F.matmul(neighbor_feature, self.weight)
        if self.use_bias:
...
class sageConv(F.nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim,
        ...
        self.weight = F.Parameter(F.Tensor(self.input_dim, self.hidden_dim))
    ...
    def forward(self, src_node_features, neighbor_node_features):
        ...
        self_hidden = F.matmul(src_node_features, self.weight)

        if self.aggr_hid_method == "sum":
            hidden = self_hidden + neighbor_hidden
        elif self.aggr_hid_method == "concat":
            hidden = F.cat([self_hidden, neighbor_hidden], dim=1)
        else:
...
class GraphSage(F.nn.Module, object):
